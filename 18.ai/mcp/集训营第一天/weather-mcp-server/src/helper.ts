...
// 🌤️ 日期格式化工具
export function formatDate(text: string): string | null {
...
// 🌍 获取城市 ID
export async function getCityLocation(city: string): Promise<string | null> {
...
export async function getWeather(city: string, date: string): Promise<string> {
  ...
  const match = data.daily.find((d: any) => d.fxDate === dateStr);
  if (!match) {
